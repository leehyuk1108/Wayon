...
internal object WayonDriveTransitionPolicy {
    const val ONROAD_REFRESH_DELAY_MS = 2_000L
    const val OFFROAD_REFRESH_DELAY_MS = 5_000L

    fun refreshDelayMs(previous: Boolean?, current: Boolean): Long? = when {
        previous == null || previous == current -> null
        current -> ONROAD_REFRESH_DELAY_MS
        else -> OFFROAD_REFRESH_DELAY_MS
    }
}
