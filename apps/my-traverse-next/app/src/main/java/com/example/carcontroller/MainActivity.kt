...
class MainActivity : AppCompatActivity(), CoroutineScope by CoroutineScope(Dispatchers.Main) {
    ...
    private fun observeWayonDriveState(onroad: Boolean) {
        ...
        if (previous == true && !onroad) requestHealthBridgeWatchFlush()
    ...
    private fun requestHealthBridgeWatchFlush() {
        runCatching {
            sendBroadcast(
                Intent("com.ijonghyeog.healthbridge.FLUSH_WATCH")
                    .setPackage("com.ijonghyeog.healthbridge"),
            )
            Log.i("WayonTransition", "Requested Health Bridge watch flush after drive")
        }.onFailure { error ->
            Log.w("WayonTransition", "Health Bridge watch flush request failed", error)
        }
    }
