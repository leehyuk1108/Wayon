...
class ExampleUnitTest {
    ...
    fun wayonTransitionRefresh_onlyRunsAfterRealStateChanges() {
        ...
        assertNull(WayonDriveTransitionPolicy.refreshDelayMs(false, false))
        assertEquals(
            WayonDriveTransitionPolicy.ONROAD_REFRESH_DELAY_MS,
            WayonDriveTransitionPolicy.refreshDelayMs(false, true),
        )
        assertEquals(
            WayonDriveTransitionPolicy.OFFROAD_REFRESH_DELAY_MS,
            WayonDriveTransitionPolicy.refreshDelayMs(true, false),
        )
    }
    ...
    @Test
    fun cloudFeed_preservesDriveAnalysisAndHealthSensorSummary() {
        val trip = JSONObject()
            .put("id", "trip-sensors")
            .put("started_at", "2026-09-04T04:00:00Z")
            .put("ended_at", "2026-09-04T04:30:00Z")
            .put("duration_s", 1800)
            .put("distance_m", 24000)
            .put("analysis", JSONObject().put("schemaVersion", "wayon-drive-v1"))
            .put("health", JSONObject()
                .put("available", true)
                .put("sensors", JSONObject()
                    .put("available", true)
                    .put("eda", JSONObject().put("arousalProxy0To100", 42))))
        val feed = WayonCloudFeedParser.parse(JSONObject()
            .put("trips", JSONArray().put(trip))
            .put("snapshots", JSONArray())
            .toString())

        val historyJson = JSONArray(feed.historyJson).getJSONObject(0)
        assertEquals("wayon-drive-v1", historyJson.getJSONObject("analysis").getString("schemaVersion"))
        assertEquals(
            42,
            historyJson.getJSONObject("health").getJSONObject("sensors")
                .getJSONObject("eda").getInt("arousalProxy0To100"),
        )
    }
}
