...
$('settings-button').onclick=()=>{showSheet('미리보기 설정',`<button class="sheet-choice" id="theme-toggle">${icon('moon')}<span><b>밝은 화면 / 어두운 화면</b><small>색상과 가독성 비교</small></span></button><button class="sheet-choice" id="size-toggle">${icon('type')}<span><b>기본 글씨 / 200% 글씨</b><small>크게 읽어도 정보를 놓치지 않도록</small></span></button><p class="inline-note">이 화면은 실제 계정·차량과 연결되지 않았어요.</p>`);$('theme-toggle').onclick=()=>{document.body.classList.toggle('dark');map?.setStyle(document.body.classList.contains('dark')?'https://tiles.openfreemap.org/styles/dark':'https://tiles.openfreemap.org/styles/positron')};$('size-toggle').onclick=()=>{document.body.classList.toggle('large-text');document.documentElement.style.fontSize=document.body.classList.contains('large-text')?'32px':'16px'}};
document.querySelectorAll('[data-navigate]').forEach(b=>b.onclick=()=>navigate(b.dataset.navigate));
document.querySelector('.wordmark').onclick=e=>{e.preventDefault();navigate('now')};
