...
이번 변경은 Wayon/Sunnypilot의 `16da7f366` 위에 적용하며, 최근 정차·출발 완화와 40 km/h 이상 가속 상한 변경을 유지한다.
...
운전자가 보고한 `정차 요구 중` 버튼 차단을 해소하기 위해, 완전 정차에서 누른 명시적 요청에만 짧은 크리프 단계를 추가했다. UI는 `브레이크 해제 후 재출발`을 표시한다. 자동 앞차 출발 경로의 정차 요구 조건은 유지한다.

- 양의 가속 명령을 만들지 않고 GM 가스 0·마찰제동 0을 생성한다. 실제 움직임을 기다리며 RES를 무한 대기하는 조건은 추가하지 않는다. 첫 브레이크 0 명령 후 기존 200 ms·새 순정 프레임·RES 5회 경로를 따른다.
- 이 수동 예외는 요청 후 2초 이내다. 속도 0.5 m/s 이상, 누적 이동 추정 0.5 m 이상, 후진 속도 또는 잘못된 관측을 감지하면 정차 제어로 복귀한다. 이는 감지 기준이며 실제 이동 거리나 정지 거리의 보장이 아니다.
- 최신 carState/selfdriveState/longitudinalPlan/radarState/modelV2가 필요하다. FCW 또는 두 lead 중 현재/2초 후 예상 간격이 4 m 이하인 경우에는 요청을 차단하며, 시도 도중 조건이 바뀌어도 중단한다.
- 계획기가 출발을 허용하면 정상 출발 제어로 넘기고 수동 예외를 닫는다. 그 뒤의 새 정차 요구는 즉시 정상적으로 처리한다.
- PCM이 ACTIVE로 바뀌어도 계획기가 계속 정차를 요구하는 동안에는 정상 출발 성공으로 판정하지 않는다. 2초가 지나면 정차로 돌아가며 자동 반복하지 않는다.
...
6. 요청 후 2초 이내 수용되지 않거나 수동 크리프 예외 밖에서 정차 요구·제어 조건 변화가 발생하면 기존 정차 제어로 돌아간다. 실패한 정차에서 자동으로 무한 반복하지 않는다. 물리 버튼과 페달이 우선한다.
