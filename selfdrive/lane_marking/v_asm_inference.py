"""CarrotPilot V-ASM wide-camera blindspot model inference."""
...
try:
  ...
  if error.name == "cv2":
    raise SystemExit("V-ASM requires OpenCV") from None
  raise
...
HYSTERESIS_ON = 0.65
HYSTERESIS_OFF = 0.25
DEFAULT_MODEL_PATH = Path(__file__).resolve().parent / "models" / "v_asm_model.onnx"
