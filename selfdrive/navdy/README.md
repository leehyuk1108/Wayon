...
제어가 동작할 때만 오른쪽에 순정 ACC의 현재 실제 설정 속도를 흰색으로 표시한다. 실제 설정값이
제어 목표보다 낮으면 위쪽, 높으면 아래쪽을 가리키는 주황색 화살표가 0.9초 주기로 페이드
인/아웃한다. 두 값이 같아지면 화살표 대신 실제 설정 속도 바로 아래에 짧은 초록색 완료 바를
표시한다. `automaticAccActive`가 꺼지면 실제 설정 속도, 화살표, 완료 바를 모두 숨긴다.
