...
def test_navdy_ambient_overspeed_uses_hysteresis_and_two_level_day_warning():
  patch = Path(__file__).parent / "hud_patch" / "engaged-path-v7-alert-banner-speed-warning"
  ...
  assert "mDayWarningDimmed ? dimLevel : brightness" in source
  assert "mDayWarningDimmed = !mDayWarningDimmed" in source
  assert "FADE_STEPS" not in source
  ...
  assert "DAY_WARNING_DIM_PERCENT:I = 0x2d" in controller
  assert "DAY_WARNING_STEP_INTERVAL_MS:J = 0x2bcL" in controller
...
def test_navdy_vehicle_geometry_marks_selected_nonclosing_cutin():
...
def test_navdy_vehicle_geometry_does_not_mark_unselected_cutin_candidate():
  ...
  radar_state = SimpleNamespace(
    leadOne=SimpleNamespace(status=True, radar=True, radarTrackId=12),
    leadTwo=SimpleNamespace(status=False),
    leadCutInRisk=SimpleNamespace(
      status=True, radar=True, radarTrackId=11, dRel=18.0, yRel=3.0,
      vRel=-4.0, vLat=0.5, score=0.6),
...
def test_navdy_path_renderer_animates_dashed_lanes_and_keeps_road_edges_solid():
  ...
  assert 'type == null || "unknown".equals(type)' not in java
