...
ONROAD_DELAY = 2.5
FADE_DURATION = 0.55
SCREEN_WAKE_FADE_DURATION = 0.85
SCREEN_SLEEP_FADE_DURATION = 2.0
OFFROAD_SNAP_EPS = 2.0
...
class MiciMainLayout(Scroller):
  ...
  def _setup_callbacks(self):
    ...
    self._car_onroad_layout.set_enabled(lambda: self.enabled and self._active_surface == SURFACE_ONROAD and not self._is_transitioning())
    self._body_onroad_layout.set_enabled(lambda: self.enabled and self._active_surface == SURFACE_ONROAD and not self._is_transitioning())
  ...
  def _snap_offroad_scroll(self, force: bool = False):
    ...
    if force or panel.state == ScrollState.STEADY:
      self._scroller.scroll_to(target_layout.rect.x, smooth=True, block_widget_interaction=True)
  ...
  def _screen_sleep_fade_alpha(self) -> float:
    interaction_time = getattr(device, "_interaction_time", -1.0)
    if not device.awake or ui_state.started or PC or interaction_time <= 0:
      return 0.0

    time_remaining = interaction_time - time.monotonic()
    if time_remaining >= SCREEN_SLEEP_FADE_DURATION:
      return 0.0

    progress = 1.0 - max(0.0, min(1.0, time_remaining / SCREEN_SLEEP_FADE_DURATION))
  ...
  def _update_trip_tracking(self):
    if ui_state.started and not ui_state.is_body:
